<div class="leagueUsers index">
<h2><?php __('LeagueUsers');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('usename');?></th>
	<th><?php echo $paginator->sort('password');?></th>
	<th><?php echo $paginator->sort('lName');?></th>
	<th><?php echo $paginator->sort('discription');?></th>
	<th><?php echo $paginator->sort('publicKey');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($leagueUsers as $leagueUser):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $leagueUser['LeagueUser']['id']; ?>
		</td>
		<td>
			<?php echo $leagueUser['LeagueUser']['usename']; ?>
		</td>
		<td>
			<?php echo $leagueUser['LeagueUser']['password']; ?>
		</td>
		<td>
			<?php echo $leagueUser['LeagueUser']['lName']; ?>
		</td>
		<td>
			<?php echo $leagueUser['LeagueUser']['discription']; ?>
		</td>
		<td>
			<?php echo $leagueUser['LeagueUser']['publicKey']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action' => 'view', $leagueUser['LeagueUser']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action' => 'edit', $leagueUser['LeagueUser']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action' => 'delete', $leagueUser['LeagueUser']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $leagueUser['LeagueUser']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class' => 'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New LeagueUser', true), array('action' => 'add')); ?></li>
	</ul>
</div>
